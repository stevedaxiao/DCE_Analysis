...
void SignalIntensityToConcentrationImageFilter<TInputImage, TMaskImage, TOutputImage>::GenerateData()
{
  ...
  InputMaskConstIterType T1MappingVolumeIter;
  if (this->GetT1Mapping())
    {
    T1MappingVolumeIter = InputMaskConstIterType(this->GetT1Mapping(),this->GetT1Mapping()->GetRequestedRegion() );
    T1MappingVolumeIter.GoToBegin();
    }
  ...
  while (!oit.IsAtEnd() )
    ...
    // if we have both ROI and T1 mapping, and T1Pre is set to T1 mapping result, check if we need to skip this voxel
    if( this->GetROIMask() && this->GetT1Mapping() && (this->GetROIMask()->GetBufferedRegion().GetSize()[0])!=0)
      {
      if(!roiMaskVolumeIter.Get())
        {
        T1Pre = 0;
    ...
    // if we have both an AIF mask and T1 mapping, use T1 mapping as T1Pre; even if this voxel was unselected by ROI,
    // select it for AIF calculation if non-0
    if( this->GetAIFMask() && this->GetT1Mapping() && (this->GetAIFMask()->GetBufferedRegion().GetSize()[0])!=0)
      {
      if(aifMaskVolumeIter.Get())
        {
          {
    ...
    // if we have T1 mapping, use T1 mapping result as T1Pre
    if( this->GetT1Mapping() && (this->GetT1Mapping()->GetBufferedRegion().GetSize()[0])!=0)
      {
        {
        T1Pre = T1MappingVolumeIter.Get();
        }
