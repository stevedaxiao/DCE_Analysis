...
public:
  ...
  // Set a T1 Mapping image for T1
  void SetT1Mapping(InputMaskType* T1MappingVolume)
  {
    this->SetNthInput(3, const_cast<InputMaskType*>(T1MappingVolume));
  }

  // Get the mask image specifying the location of voxels for model fit.
  const InputMaskType* GetT1Mapping() const
  {
