...
int DoIt( int argc, char * argv[], const T1 &, const T2 &)
{
  ...
  //Read T1 Mapping
  typename MaskVolumeReaderType::Pointer T1MappingVolumeReader = MaskVolumeReaderType::New();
  typename MaskVolumeType::Pointer T1MappingVolume = 0;
  if (T1MappingFileName != "")
    {
    T1MappingVolumeReader->SetFileName(T1MappingFileName.c_str() );
    T1MappingVolumeReader->Update();
    T1MappingVolume = T1MappingVolumeReader->GetOutput();
    }
  ...
  if(T1MappingFileName != "")
    {
    converter->SetT1Mapping(T1MappingVolume);
    }
